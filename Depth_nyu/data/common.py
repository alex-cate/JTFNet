...
def get_patch(depth_lr, depth_hr, color, patch_size, scale):
    ih, iw = depth_lr.shape[:2]
    ...
    depth_lr = depth_lr[iy:iy + ip, ix:ix + ip, :]
    depth_hr = depth_hr[ty:ty + tp, tx:tx + tp, :]
    color = color[ty:ty + tp, tx:tx + tp, :]

    return depth_lr, depth_hr, color
