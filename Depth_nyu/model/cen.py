...
class KG(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, bias=True, act=nn.PReLU()):
        ...
        self.conv_first = nn.Sequential(conv(n_feat, n_feat, 3, padding=1, bias=bias), act,
                                        ...
                                        conv(n_feat, n_feat, 3, padding=1, bias=bias), act,
                                        conv(n_feat, n_feat, 3, padding=1, bias=bias)
                                        )
        self.tensor=conv(n_feat, kernel_size * kernel_size * 32, 1, padding=0, bias=bias)
...
class JBF(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, act=nn.PReLU()):
        ...
        self.bottleneck = nn.Sequential(conv(n_feat, 32, kernel_size=1, padding=0), act)
        ...
        self.jbf_conv = nn.Sequential(conv(32, n_feat, 3, padding=1), act, conv(n_feat, n_feat, 3, padding=1))
    ...
    def forward(self, image, guidance):
        ...
        b, c, h, w = image_ten.shape
        patch = self.unfold(image_ten).view(b, c * self.kernel_size * self.kernel_size, h, w)
...
class EGEM(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, tensor_num, bias=True, act=nn.PReLU()):
        ...
        self.jbf = JBF(conv, n_feat, kernel_size, act=nn.PReLU())
...
class CEN(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        self.sub_mean2 = common.MeanShift(args.rgb_range, rgb_mean, rgb_std)
        ...
        for i in range(n_groups):
            ...
            self.rdbs.append(common.RDB(growRate0=G0, growRate=G, nConvLayers=C))
        ...
        self.image_up = common.Upsampler(conv, scale, n_feats, act=False)
        self.edge_up = common.Upsampler(conv, scale, n_feats, act=False)
        # self.fusion = conv(n_feats * (n_groups + 1), n_feats, 1, padding=0)
        self.fusion = conv(n_feats * n_groups, n_feats, 1, padding=0)
        self.jbf = JBF(conv, n_feats, kernel_size, act=act)
        ...
        self.add_mean2 = common.MeanShift(args.rgb_range, rgb_mean, rgb_std, 1)
    ...
    def forward(self, image, edge):
        ...
        for i in range(self.n_groups):
            ...
            rdb_ten=self.rdbs[i](rdb_ten)
            rdb_concat.append(rdb_ten)
            rdb_cat = torch.cat(rdb_concat, 1)
            edge_ten = self.egems[i](edge_ten, rdb_ten)
            # edge_ten = self.egems[i](edge_ten, down_ten)
        # up_ten = self.image_up(down_ten)
        # up_concat.append(up_ten)
        # img_up = torch.cat(up_concat, 1)
        # img_up = self.fusion(img_up)
        img_fus = self.fusion(rdb_cat)
        img_up=self.image_up(img_fus)
        edge_up = self.edge_up(edge_ten)
        img_fus = self.jbf(img_up, edge_up)
