...
class MSG(nn.Module):
    ...
    def forward(self, depth, color):
        ...
        color_ten_x2 = self.color_fun_x2(color)

        depth_ten_x2 = torch.cat([depth, color_ten_x2], dim=1)
        depth_ten_x4 = self.sr_stage_x2(depth_ten_x2)

        depth_ten_x4 = torch.cat([depth_ten_x4, color], dim=1)
        depth_ten_x4 = self.sr_stage_x4(depth_ten_x4)

        # print(depth_ten_x8.shape)

        depth_sr = self.depth_recon(depth_ten_x4)
