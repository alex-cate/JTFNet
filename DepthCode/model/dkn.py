...
class DKN(nn.Module):
    ...
    def forward(self, depth, image):
        depth = F.interpolate(depth, scale_factor=self.scale, mode='bicubic', align_corners=False)

        weight, offset = self._shift_and_stitch(depth, image)
    ...
    def _shift_and_stitch(self, depth, image):
        ...
        for i in range(4):
            for j in range(4):
                ...
                img_shift = m(image)
                depth_shift = m(depth)
                w, o = self._infer(depth_shift, img_shift)
...
class FDKN(nn.Module):
    ...
    def forward(self, depth, image):
        ...
        # print(imkernel.shape)
        # print(depthkernel.shape)
